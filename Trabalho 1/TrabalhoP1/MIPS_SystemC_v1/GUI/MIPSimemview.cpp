...
QString MIPSimemview::MemItem(unsigned int i)
{
    ...
    //address
    HexVal.setNum(i,16);
    ListStr+=HexVal.rightJustify(4,'0')+":";

    //Instruction code
    instVal=mem.at(i);
    HexVal.setNum((unsigned) instVal,16);
    ListStr=ListStr+HexVal.rightJustify(8,'0');
...
void MIPSimemview::initList()
{
    ...
    if(count()>0) {
       ...
       // set Maximum size
       setMaximumSize(maxWidth+40,height*mem.size()/4+40);

       if(mem.size()>0 && mem.size()<80)
          resize(maxWidth+40,height*mem.size()/4+40);
       else resize(maxWidth+40,100);
    }
}
