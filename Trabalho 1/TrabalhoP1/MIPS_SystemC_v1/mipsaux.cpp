...
void disassemble(unsigned cod, char *decod)
{
    ...
    if(opcode==0) { //R-Format
	  ...
	  /* some are missing here */
          ...
          /* some are missing here */
          case 48: strcpy(inst,"lrl"); break;
          case 49: strcpy(inst,"pct"); break;
	  default: strcpy(decod,"unknown"); return;
       }
       if (funct == 48)
          sprintf(decod,"%s $%d,$%d,%d",inst,rd,rt,shamt);
          else if (funct == 49)
                  sprintf(decod,"%s $%d,$%d",inst,rd,rt);
                  else sprintf(decod,"%s $%d,$%d,$%d",inst,rd,rs,rt);
